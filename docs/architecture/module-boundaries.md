...
| Calculation engine   | Deterministic formula evaluation over fully resolved JSON input                                                                    | `@niedax/domain`, reviewed static rule manifest                                                                                     | Next.js, React, Fastify, server actions, HTTP, ORM, DB, repositories, filesystem, network, environment, clock, random IDs | `calculate`, `calculateV2`, `calculationEngineV2`; owns formula implementation only                         | Unit, property, golden, schema, trace, determinism, and forbidden-import tests                             |
